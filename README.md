...
- 职业记忆：自动汇总职业定位、能力、项目证据、偏好和训练缺口，支持固定长期陪练会话与 `AI_CONTEXT.md` 导出。
- 多模型接入：预置 DeepSeek、OpenAI、通义千问、智谱、Moonshot、Dify、本地 Ollama 和自定义 OpenAI-compatible 接口。
...
- 云端模型的免费额度、模型权限和计费由对应服务商决定；ChatGPT 会员不等同于 OpenAI API 额度。本地 Ollama 可在不调用云 API 的情况下使用兼容模型。
