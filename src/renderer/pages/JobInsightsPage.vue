...
<template>
  <section>
    ...
    <div v-if="jobs.length" class="insights-grid">
      ...
      <section class="insight-panel"><header><span><Sparkles :size="17" /><strong>技能热度</strong></span><small>岗位关键词</small></header><div class="skill-cloud"><span v-for="item in skillStats" :key="item.label"><b>{{ item.label }}</b><small>{{ item.value }}</small></span><p v-if="!skillStats.length">等待完整岗位描述后提取技能。</p></div></section>
