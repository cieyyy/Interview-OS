...
const { store, saveKnowledge, deleteKnowledge } = useWorkspace();
const search = ref('');
...
const types: Array<{ value: KnowledgeType; label: string }> = [
  ...
  { value: 'project', label: '项目知识' }, { value: 'note', label: '普通笔记' }
];
  ...
  <section>
    ...
    <div class="workspace-layout">
      ...
      <form class="editor-panel" data-testid="knowledge-form" @submit.prevent="submit">
        ...
        <div class="form-actions"><button v-if="selectedId" class="button danger ghost" type="button" @click="remove">删除</button><span></span><button class="button primary" type="submit" data-testid="knowledge-save">保存知识卡片</button></div>
      </form>
    </div>
  </section>
</template>
