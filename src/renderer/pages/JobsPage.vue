  ...
  <section>
    ...
    <div v-else class="jobs-layout">
      <aside class="collection-panel jobs-list">
        ...
        <p v-if="!store.workspace?.jobs.length" class="list-empty">还没有目标岗位描述</p>
      </aside>
      <article v-if="selected" class="panel job-detail" data-testid="job-detail">
        <div class="job-title"><div><span class="eyebrow">{{ selected.company || 'TARGET JOB' }}</span><h2>{{ selected.title }}</h2></div><span class="chip accent">{{ selectedAnalysis?.requirements.length ?? selected.requirements.length }} 项能力</span></div>
