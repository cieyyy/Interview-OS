...
const dimensions = computed<Array<{ key: keyof JobMatchDimensions; label: string; value: number }>>(() => {
  const value = selectedJob.value?.matchDimensions;
  if (!value) return [];
  return [
  ...
  <section>
    ...
    <div class="skill-graph-layout">
      ...
      <aside class="skill-analysis-side">
        ...
        <section class="job-audit-panel"><header><ShieldAlert :size="17" /><div><span class="eyebrow">TRUST AUDIT</span><h3>岗位审计</h3></div></header><template v-if="selectedJob"><div class="audit-scores"><span><strong>{{ selectedJob.qualityScore }}</strong><small>字段质量</small></span><span><strong>{{ selectedJob.trustScore }}</strong><small>可信度</small></span></div><div class="audit-flags"><span v-for="flag in selectedJob.riskFlags" :key="flag" class="risk">{{ flag }}</span><span v-for="flag in selectedJob.biasFlags" :key="flag" class="bias">{{ flag }}</span><small v-if="!selectedJob.riskFlags.length && !selectedJob.biasFlags.length">未发现明显风险或偏见表述。</small></div></template><p v-else>选择岗位后查看信息完整度、风险和公平性提示。</p></section>
      </aside>
    </div>
